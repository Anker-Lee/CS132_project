classdef Test_func_Query < matlab.uitest.TestCase
    properties
        QueryUI;
        UserBackend;
        train_state_UI
    ...
    methods (TestMethodSetup)
        function launchApp(testcase)
            ...
            testcase.UserBackend = user_backend('1',train_db,testcase.time);
            testcase.QueryUI = testcase.UserBackend.QueryUI;
        end
    end
    methods(Test)
        %t3.3
        function test_query(testcase)
            %t3.3.1
            testcase.choose(testcase.QueryUI.DropDown_3,'上海');
            pause(2);
            testcase.choose(testcase.QueryUI.DropDown_4,'苏州');
            pause(2);
            testcase.press(testcase.QueryUI.Button_4);
            pause(2);
            testcase.press(testcase.UserBackend.TrainUI.Button);
            pause(2);
            %t3.3.2
            testcase.choose(testcase.UserBackend.QueryUI.DropDown_3,'上海');
            pause(2);
            testcase.choose(testcase.UserBackend.QueryUI.DropDown_4,'上海');
            pause(1);
            testcase.press(testcase.UserBackend.QueryUI.Button_4);
            pause(2);
            %t3.3.3
            testcase.type(testcase.UserBackend.Time_backend.UI.EditField,16);
            pause(2);
            testcase.choose(testcase.UserBackend.QueryUI.DropDown_4,'苏州');
            pause(2);
            testcase.press(testcase.UserBackend.QueryUI.Button_4);
        end
        function test_query_sort(testcase)
            %t3.3.4
            testcase.choose(testcase.QueryUI.DropDown_3,'上海');
            pause(2);
            testcase.choose(testcase.QueryUI.DropDown_4,'苏州');
            testcase.press(testcase.QueryUI.Button_4);
            pause(2);
            testcase.press(testcase.UserBackend.TrainUI.Button_2);
            pause(2);
            testcase.press(testcase.UserBackend.TrainUI.Button_3);
            pause(2);
            testcase.press(testcase.UserBackend.TrainUI.Button_4);
        end        
    end
end