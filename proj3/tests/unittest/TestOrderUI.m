classdef TestOrderUI < matlab.uitest.TestCase
    properties
        QueryUI;
        UserBackend;
        train_state_UI
    ...
    methods (TestMethodSetup)
        function launchApp(testcase)
            ...
            time = back(train_db,testcase.train_state_UI);
    ...
    methods(Test)
        %t1.8.1
        % T1.8.1.1
        function test_show1(testcase)
            testcase.QueryUI.find_train('上海','杭州');
            testcase.press(testcase.UserBackend.TrainUI.buttons(2));
            delete(testcase.UserBackend.TrainUI);
            delete(testcase.train_state_UI);
            delete(testcase.UserBackend.user_state_ui);
            delete(testcase.UserBackend.OrderUI);
        end
        %t1.8.1.2
        function test_show2(testcase)
            testcase.QueryUI.find_train('上海','杭州');
            testcase.press(testcase.UserBackend.TrainUI.buttons(5));
            delete(testcase.UserBackend.TrainUI);
            delete(testcase.train_state_UI);  
            delete(testcase.UserBackend.user_state_ui);
            delete(testcase.UserBackend.OrderUI);
